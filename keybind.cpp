...
const std::string keynames[] = {
	"", XOR("[M1]"), XOR("[M2]"), XOR("[CANCEL]"), XOR("[M3]"), XOR("[M4]"), XOR("[M5]"), XOR("[7]"),
	XOR("[BS]"), XOR("[TAB]"), XOR("[10]"), XOR("[11]"), XOR("[NUM 5]"), XOR("[ENTER]"), XOR("[14]"), XOR("[15]"), XOR("[SHIFT]"),
...
void Keybind::draw() {
	...
	if (!m_set && !(m_key >= 0 && m_key <= 0xFE))
		render::menu.string(p.x - render::menu.size(XOR("[NONE]")).m_width + m_w + 1, p.y + 15 + 4 - 21, { 150,150,150,m_parent->m_alpha }, XOR("[NONE]"));

	if (!m_set && m_key >= 0 && m_key <= 0xFE)
		render::menu.string(p.x - render::menu.size(XOR("")).m_width + m_w + 1, p.y + 15 + 4 - 21, { 150,150,150,m_parent->m_alpha }, XOR(""));

	// todo; animate with '...'
	if (m_set)
		render::menu.string(p.x - render::menu.size(XOR("[-]")).m_width + m_w + 1, p.y + 15 + 4 - 21, { 255, 0, 0 }, XOR("[-]"));

	// we have a key assigned.
	else if (m_key >= 0 && m_key <= 0xFE)
		render::menu.string(p.x - render::menu.size(tfm::format("%s", keynames[m_key])).m_width + m_w + 1, p.y + 15 + 4 - 21, { 150,150,150,m_parent->m_alpha }, tfm::format("%s", keynames[m_key]));
}
...
void Keybind::click() {
	...
	Rect bind = { p.x , p.y - 2, m_w + 20, KEYBIND_BOX_HEIGHT };
