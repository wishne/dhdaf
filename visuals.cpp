...
void Visuals::ModulateWorld() {
	...
	// night
	Color ref = g_menu.main.visuals.worldcolor.get();

	const float darkness = 1.f - (g_menu.main.visuals.worlddarkness.get() / 100.f);

	const int r = ref.r() * darkness;
	const int g = ref.g() * darkness;
	const int b = ref.b() * darkness;

	const int sr = r * r;
	const int sg = g * g;
	const int sb = b * b;

	Color world_clr = { sr / 255, sg / 255, sb / 255, 255 };
	Color props_clr = { r, g, b, 255 };

	if (g_menu.main.visuals.world_modulation.get()) {
		...
		for (const auto& p : props) {
			...
			p->AlphaModulate(g_menu.main.visuals.transparent_props.get() ? (g_menu.main.visuals.transparent_props_amount.get() / 100.f) : 1.f);
		}
...
void Visuals::ThirdpersonThink() {
	...
	const float distance = g_menu.main.visuals.thirdperson_distance.get();

	// camera should be in thirdperson.
	if (m_thirdperson) {

		// if alive and not in thirdperson already switch to thirdperson.
		if (g_cl.m_local->alive() && !g_csgo.m_input->CAM_IsThirdPerson())
...
void Visuals::Hitmarker() {
	float current_hit_time = -1.f;

	for (auto it = m_impacts.rbegin(); it != m_impacts.rend(); ++it) {
		Impact* impact = &*it;
		if (impact->m_hit_player) {
			current_hit_time = impact->m_time;
			break;
		}
	}
	...
	for (auto& impact : m_impacts) {
		...
		float complete = (g_csgo.m_globals->m_curtime - impact.m_time) / 1.f;
		...
		Color clr = Color(255, 255, 255, alpha);

		if (g_menu.main.misc.hitmarker.get(0) && impact.m_time == current_hit_time) {
			pos.x = g_cl.m_width / 2.f;
			pos.y = g_cl.m_height / 2.f;

			render::line(pos.x - line, pos.y - line, pos.x - (line / 2.f), pos.y - (line / 2.f), clr);
			render::line(pos.x - line, pos.y + line, pos.x - (line / 2.f), pos.y + (line / 2.f), clr);
			render::line(pos.x + line, pos.y + line, pos.x + (line / 2.f), pos.y + (line / 2.f), clr);
			render::line(pos.x + line, pos.y - line, pos.x + (line / 2.f), pos.y - (line / 2.f), clr);
		}

		if (render::WorldToScreen(impact.m_hit_pos, pos) && game::IsValidHitgroup(impact.m_hit_group) && g_menu.main.misc.hitmarker.get(1)) {
			render::line(pos.x - line, pos.y - line, pos.x - (line / 2.f), pos.y - (line / 2.f), clr);
			render::line(pos.x - line, pos.y + line, pos.x - (line / 2.f), pos.y + (line / 2.f), clr);
			render::line(pos.x + line, pos.y + line, pos.x + (line / 2.f), pos.y + (line / 2.f), clr);
			render::line(pos.x + line, pos.y - line, pos.x + (line / 2.f), pos.y - (line / 2.f), clr);
...
void Visuals::think() {
	...
	for (int i = 1; i <= g_csgo.m_globals->m_max_clients; i++) {
		...
		g_resolver.Override(&data->m_records.front());

		m_render_override = true;
	}
	...
	for (auto i = m_impacts.begin(); i != m_impacts.end();) {
		auto impact = *i;

		if (abs(g_csgo.m_globals->m_curtime - impact.m_time) > 1.f) {
			i = m_impacts.erase(i);
			continue;
		}

		if (g_menu.main.visuals.impacts.get())
			render::sphere(impact.m_pos, 2.5f, 5.f, 1.f, g_menu.main.visuals.impact_color.get());
...
void Visuals::StatusIndicators() {
	...
	if (g_menu.main.visuals.indicators.get(0)) {
		...
		Indicator_t ind{ };
		ind.color = change > 35.f ? clr[0] : clr[1];
		ind.text = XOR("LBY");
		indicators.push_back(ind);
	}

	// EXT
	if (g_menu.main.visuals.indicators.get(2) && (g_aimbot.m_fake_latency || g_menu.main.misc.force_fake_latency.get())) {
		Indicator_t ind{ };
		ind.color = g_aimbot.m_fake_latency ? 0xffffffff : clr[0];
	...
	if (g_menu.main.visuals.indicators.get(3) && g_aimbot.m_override) {
		Indicator_t ind{ };
	...
	if (g_menu.main.visuals.indicators.get(4) && g_aimbot.m_baim) {
		Indicator_t ind{ };
		ind.color = 0xffffffff;
		ind.text = XOR("BAIM");
	...
	int x, y;
	g_csgo.m_engine->GetScreenSize(x, y);

	x /= 2;
	y /= 2;

	if (g_menu.main.visuals.manual_antiaim.get()) {
		bool arrow = g_menu.main.visuals.manual_antiaim.get() == 2;

		Color color = {};

		switch (g_hvh.m_manual_mode) {
		case 0:
			color = g_menu.main.visuals.manual_antiaim_color.get();
			break;
		case 1:
			color = g_menu.main.visuals.manual_antiaim_color1.get();
			break;
		case 2:
			color = g_menu.main.visuals.manual_antiaim_color2.get();
			break;
		case 3:
			color = g_menu.main.visuals.manual_antiaim_color3.get();
			break;
		}

		int offset = 50;

		if (g_movement.m_autopeek && !g_movement.m_reached_target)
			color = Color(255,255,255);

		render::FontSize_t size[] = { render::arrows.size("X"),render::arrows.size("Y"),render::arrows.size("Z"),render::arrows.size("W") };

		if (g_hvh.m_left)
			render::arrows.string(x - 50 - (size[0].m_width / 2.f), y - (size[0].m_height / 2.f), color, "X");

		if (g_hvh.m_back)
			render::arrows.string(x - (size[1].m_width / 2.f), y + 50 - (size[1].m_height / 2.f), color, "Y");

		if (g_hvh.m_right)
			render::arrows.string(x + 50 - (size[2].m_width / 2.f), y - (size[2].m_height / 2.f), color, "Z");

		if (g_hvh.m_front)
			render::arrows.string(x - (size[3].m_width / 2.f), y - 50 - (size[3].m_height / 2.f), color, "W");
	}
	...
	for (size_t i{ }; i < indicators.size(); ++i) {
		...
		render::indicator.string(15, (g_cl.m_height / 2.f) + (22 * i), indicator.color, indicator.text);
	}
}
...
void Visuals::PenetrationCrosshair() {
	...
	if (!g_menu.main.visuals.pen_crosshair.get() || !g_cl.m_local || !g_cl.m_processing)
		return;
...
void Visuals::OffScreen(Player* player, int alpha) {
	...
	if (!g_cl.m_local || !g_cl.m_processing || !g_cl.m_local->enemy(player))
		return;
	...
	if (!is_on_screen
		...
		render::triangle(verts[0].m_pos, verts[2].m_pos, verts[1].m_pos, color, true);
...
void Visuals::RenderOverrideArrow(Player* player, bool enemy) {
	AimPlayer* data = &g_aimbot.m_players[player->index() - 1];
	if (!data || data->m_records.empty())
		return;

	OverrideData* override_data = &g_resolver.m_override_data[player->index() - 1];

	if (!override_data)
		return;

	if (!enemy || !g_cl.m_processing || !m_render_override || !g_cl.m_override_resolver_toggle)
		return;

	float best_fov = FLT_MAX;
	for (int idata = 0; idata < 64; idata++) {
		if (g_resolver.m_override_data[idata].m_fov < best_fov)
			best_fov = g_resolver.m_override_data[idata].m_fov;
	}

	Color clr = data->m_records.front().m_mode == Resolver::Modes::RESOLVE_FLICK ? Color(255, 255, 255, 205) : Color(168, 230, 69, 205);

	if (override_data->m_fov != best_fov || best_fov == FLT_MAX)
		return;

	// here we do arrows
	vec2_t screen_start, screen_end;

	if (render::WorldToScreen(override_data->m_pos, screen_start)) {
		if (render::WorldToScreen(override_data->m_end, screen_end)) {
			vec2_t delta = screen_end - screen_start;

			ang_t dir;
			math::VectorAngles(override_data->m_end - override_data->m_from, dir);

			render::line(screen_start, screen_end, clr);

			float ang_delta = math::AngleDiff(override_data->m_angle, dir.y);

			if (fabs(ang_delta) <= 45.f)
				render::triangle(screen_end + vec2_t(0, -16), screen_end + vec2_t(8, 0), screen_end + vec2_t(-8, 0), clr, true);

			else if (fabs(ang_delta) >= 135.f)
				render::triangle(screen_end + vec2_t(0, 16), screen_end + vec2_t(-8, 0), screen_end + vec2_t(8, 0), clr, true);

			else if (delta.x < 0)
				render::triangle(screen_end + vec2_t(-16, 0), screen_end + vec2_t(0, -8), screen_end + vec2_t(0, 8), clr, true);
...
void Visuals::DrawPlayer(Player* player) {
	...
	if (local && !g_csgo.m_input->m_camera_in_third_person)
		return;
	...
	bool bone_esp = (enemy && g_menu.main.players.skeleton.get(0)) || (!enemy && g_menu.main.players.skeleton.get(1) || (local && g_menu.main.players.skeleton.get(2)));
	if (bone_esp)
		DrawSkeleton(player, alpha);

	// is box esp enabled for this player.
	bool box_esp = ((enemy && g_menu.main.players.box.get(0)) || (!enemy && g_menu.main.players.box.get(1))) && !local;
	...
	bool name_esp = ((enemy && g_menu.main.players.name.get(0)) || (!enemy && g_menu.main.players.name.get(1))) && !local;
	...
	bool health_esp = ((enemy && g_menu.main.players.health.get(0)) || (!enemy && g_menu.main.players.health.get(1)) && !local);
	...
	if (health_esp) {
		...
		if (enemy && g_menu.main.players.custom_health.get(0))
			clr = g_menu.main.players.custom_health_col_enemy.get();

		else if (!enemy && g_menu.main.players.custom_health.get(1))
			clr = g_menu.main.players.custom_health_col_friendly.get();
		...
		// render actual bar.
		if (dormant)
			render::rect(box.x - 5, y + h - fill - 1, 2, fill + 2, { 130, 130, 130 , alpha });
		else
			render::rect(box.x - 5, y + h - fill - 1, 2, fill + 2, clr);
	...
	AimPlayer* data = &g_aimbot.m_players[player->index() - 1];
	std::set<std::string> added_flags;
	auto add_flag_if_unique = [&](const std::string& text, bool condition, const Color& color) {
		if (condition && added_flags.find(text) == added_flags.end()) {
			flags.push_back({ text, color });
			added_flags.insert(text);
		} };
	...
	if (!local)// draw flags.
	{
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			if (*it == 1) {
				// helmet and kevlar.
				if (player->m_bHasHelmet()) // if the player has a helmet then his armor would be 100%.
			...
			auto& records = g_aimbot.m_players[player->index() - 1].m_records;

			if (*it == 6 && !records.empty()) {
				LagRecord* current = &records.front();

				flags.push_back({ "FAKE", (current->m_mode == Resolver::Modes::RESOLVE_FLICK ||
					current->m_mode == Resolver::Modes::RESOLVE_WALK ||
					current->m_mode == Resolver::Modes::RESOLVE_PREFLICK) ? Color(0, 255, 0, low_alpha) : Color(255, 0, 0, low_alpha) });
			}
			...
			if (enemy) {
				bool nonehack = !data->m_is_cheese_crack && !data->m_is_kaaba && !data->m_is_dopium && !data->leak_familysadface && !data->real_familyhappyface && !data->m_is_amigo && !data->m_is_robertpaste &&
					!data->m_is_pandora && !data->m_is_fade && !data->m_is_godhook;
				add_flag_if_unique("MOORON", data->m_is_kaaba || data->m_is_pandora || data->m_is_imperial_beta || data->m_is_amigo && !data->m_is_cheese_crack && !data->m_is_dopium, { 255, 255, 255, low_alpha });
				add_flag_if_unique("FAKEFAMILY", data->leak_familysadface, { 255, 255, 255, low_alpha });
				add_flag_if_unique("FAMILY", data->real_familyhappyface, { 155, 210, 100, low_alpha });
				add_flag_if_unique("ROBERT", data->m_is_robertpaste, { 255, 255, 255, low_alpha });
				add_flag_if_unique("NAP", data->nap, { 255, 255, 255, low_alpha });
				add_flag_if_unique("W CHEAT", data->novocheat, { 255, 255, 255, low_alpha });
	...
	// draw bottom bars.
	if (!local) {
		RenderOverrideArrow(player, enemy);
		...
		if (enemy && g_menu.main.players.lby_update.get()) {
			...
			if (data && data->m_moved && data->m_records.size()) {
				...
				if (current) {
					if (current->m_abs_velocity[1].length_2d() < 1.0f && data->m_body_index <= 3) {
						// calculate box width
		...
		// draw weapon.
		if ((enemy && g_menu.main.players.weapon.get(0)) || (!local && !enemy && g_menu.main.players.weapon.get(1))) {
...
void Visuals::DrawHistorySkeleton(Player* player, int opacity) {
	...
	if (!g_cl.m_local || !g_menu.main.misc.fake_latency.get())
		return;
...
void Visuals::DrawSkeleton(Player* player, int opacity) {
	...
	for (int i{ }; i < hdr->m_num_bones; ++i) {
		...
		Color clr = enemy ? g_menu.main.players.skeleton_enemy.get() : (local ? g_menu.main.players.skeleton_local.get() : g_menu.main.players.skeleton_friendly.get());
		clr.a() = opacity;
...
void Visuals::RenderGlow() {
	...
	for (int i{ 0 }; i < g_csgo.m_glow->m_object_definitions.Count(); ++i) {
		GlowObjectDefinition_t* obj = &g_csgo.m_glow->m_object_definitions[i];
		// get player ptr.
		player = obj->m_entity->as< Player* >();

		if (!player || player->dormant() || !player->IsPlayer())
			continue;
		...
		else if (!g_menu.main.players.glow.get(2))
			continue;
		...
		obj->m_color.x = (float)color.r() / 255.0f;
		obj->m_color.y = (float)color.g() / 255.0f;
		obj->m_color.z = (float)color.b() / 255.0f;
		obj->m_alpha = ((float)color.a() / 255.0f);
		obj->m_render_occluded = true;
		obj->m_render_unoccluded = false;
	}
}

void Visuals::DrawHitboxMatrix(LagRecord* record, Color col, float time, int hitgroup) {
	const model_t* model;
	studiohdr_t* hdr;
...
void Visuals::DebugAimbotPoints(Player* player) {
	...
	if (!g_bones.BuildBones(player, BONE_USED_BY_HITBOX, matrix, true))
		return;
