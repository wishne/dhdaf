		...
		if (pressed) {
			...
			if (g_input.IsCursorInRect(tabs_area) && !f->m_tabs.empty()) {
				...
				for (size_t i{}; i < f->m_tabs.size(); ++i) {
					...
					int twidth = render::menu_shade.size(t->m_title).m_width + 10;
					int dwidth = render::menu_shade.size(t->m_desc).m_width + 10;
					Rect tab{ tabs_area.x + font_w, tabs_area.y + 5, (dwidth > twidth ? dwidth : twidth), tabs_area.h };
					font_w += ((dwidth > twidth && t == f->m_active_tab) ? dwidth : twidth);
