...
bool CSGO::init() {
	...
	SetClanTag = pattern::find(m_engine_dll, XOR("53 56 57 8B DA 8B F9 FF 15"));
	BoneCache = pattern::find(m_client_dll, XOR("FF B7 ?? ?? ?? ?? 52")).add(0x6).to< size_t >();
	PostProcessing = pattern::find(g_csgo.m_client_dll, XOR("80 3D ? ? ? ? ? 53 56 57 0F 85")).add(0x2).to<size_t>();
	ConstructMessage = (DWORD)pattern::find(m_engine_dll, XOR("56 57 8B F9 8D 4F 08 C7 07 ? ? ? ? E8 ? ? ? ? C7"));
	// exported functions.
