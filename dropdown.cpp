...
void Dropdown::draw() {
	...
	// draw label.
	if (m_use_label)
		render::menu_shade.string(p.x + 0, p.y - 2, { 205, 205, 205, m_parent->m_alpha }, m_label);
	...
	if (!m_items.empty()) {
		...
		render::menu.string(p.x + DROPDOWN_X_OFFSET + 5, p.y + m_offset + 2, { 152, 152, 152, m_parent->m_alpha }, GetActiveItem());
	}
