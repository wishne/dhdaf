...
void Shots::OnShotFire(Player* target, float damage, LagRecord* record, vec3_t end_pos) {

	ShotRecord shot;
	shot.m_i = target->index();
...
void Shots::OnImpact(IGameEvent* evt) {
	...
	g_visuals.m_impacts.emplace_back(Impact(pos, g_csgo.m_globals->m_curtime));
	...
	ImpactRecord* cur_impact = &m_impacts.front();

	cur_impact->m_spread = !trace.m_entity || trace.m_entity->index() != target->index();
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	if (!g_visuals.m_impacts.empty()) {
		g_visuals.m_impacts.back().m_hit_pos = m_impact;

		g_visuals.m_impacts.back().m_hit_group = group;
		g_visuals.m_impacts.back().m_hit_player = true;
	}

	// hitmarker.
	if (g_menu.main.misc.hitmarker.get(0) || g_menu.main.misc.hitmarker.get(1)) {
		...
		if (g_menu.main.misc.notifications.get(1)) {
			...
			std::string out = tfm::format(XOR("did -%i in %s to %s\n"), (int)damage, m_groups[group], name);
			g_notify.add(out);
		}
	}
